...
/** Create or edit an invoice: pick a customer, add lines, see totals update live. */
export function AdminInvoiceForm({ mode }: { mode: 'create' | 'edit' }) {
  ...
  useEffect(() => {
    let active = true
    Promise.all([fetchCustomers(), fetchSettings(), mode === 'edit' && id ? fetchInvoice(id) : null])
      .then(([cs, settings, existing]) => {
        if (!active) return
        setCustomers(cs)
        setDefaultVat(Number(settings.default_vat_rate) || 21)
        if (mode === 'create') {
          setDueDate(addDays(todayISO(), settings.invoice_due_days || 14))
          setLines([emptyLine(Number(settings.default_vat_rate) || 21)])
        } else if (existing) {
          const { invoice, lines: existingLines } = existing
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (mode === 'create') {
        ...
        navigate(`/admin/facturen/${created.id}`, { replace: true })
      } else if (id) {
        await updateInvoice(id, draft)
        navigate(`/admin/facturen/${id}`, { replace: true })
      }
    } catch (err) {
  ...
  const backTo = mode === 'edit' && id ? `/admin/facturen/${id}` : '/admin/facturen'
  const numberCls =
  ...
  return (
    <>
      ...
      <PageHeading kicker="Facturen" title={mode === 'create' ? 'Nieuwe factuur' : 'Factuur bewerken'} />
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        <Card className="p-6">
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <label className="flex flex-col gap-1.5">
                <span className="font-sans text-sm font-semibold text-near-black">Factuurdatum</span>
                <input
                  type="date"
                  value={issueDate}
              ...
              <label className="flex flex-col gap-1.5">
                <span className="font-sans text-sm font-semibold text-near-black">Vervaldatum</span>
                <input
                  type="date"
                  value={dueDate}
                      ...
                      {group.items.map((item) => (
                        ...
                        >
                          ...
                          <span className="shrink-0 tabular-nums text-near-black/45">
                            {item.price ? formatEUR(item.price) : 'op maat'}
                          </span>
                        </button>
                      ))}
                    </div>
          ...
          <div className="flex flex-col gap-3 sm:gap-2">
            {lines.map((line, i) => (
              ...
              >
                ...
                />
                <input type="number" step="0.01" value={line.quantity} onChange={(e) => setLine(i, { quantity: Number(e.target.value) })} className={numberCls} aria-label="Aantal" />
                <input type="number" step="0.01" value={line.unit_price} onChange={(e) => setLine(i, { unit_price: Number(e.target.value) })} className={numberCls} aria-label="Prijs" />
                <input type="number" step="1" value={line.vat_rate} onChange={(e) => setLine(i, { vat_rate: Number(e.target.value) })} className={numberCls} aria-label="Btw percentage" />
          ...
          <div className="mt-6 flex justify-end">
            <div className="w-full max-w-xs font-sans text-sm">
              ...
              <div className="mt-2 flex justify-between border-t-2 border-emerald-deep/15 pt-2.5">
                ...
                <span className="font-display text-base font-semibold text-emerald-deep tabular-nums">{formatEUR(totals.total)}</span>
              </div>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-6">
          <TextAreaField
            label="Notitie op de factuur"
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
            placeholder="Optionele toelichting die op de factuur komt te staan."
          />
        </Card>
        ...
        <div className="flex items-center gap-3">
          <PrimaryButton type="submit" disabled={saving}>
            {saving ? 'Opslaan…' : mode === 'create' ? 'Factuur aanmaken' : 'Wijzigingen opslaan'}
          </PrimaryButton>
          <Link to={backTo}>
